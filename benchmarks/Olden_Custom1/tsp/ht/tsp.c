...
extern Tree volatile tmp_a,case2_t;
extern volatile int push_flag;
extern Tree volatile g_curr_pointer;
...
static void reverse(Tree t) {
  ...
  for (t=t->next; t; back=t,t=next) {
    next = t->next;				///////---------15.18% load penalty
    t->next = back;
    back->prev = t;
    }
  tmp->next = prev;
...
static Tree merge(Tree a, Tree b, Tree t, int nproc) {
  ...
  for (a=a->next; a!=tmp; a=a->next) {   //--------22.32%=load penalty
    g_curr_pointer=a;  
    test = distance(a,t);
   // _mm_prefetch(a->next,1);
    if (test < mindist) {
      mindist = test;
      min = a;
      } /* if */
  ...
  tmp_a=b;
  push_flag=1;
  for (b=b->next; b!=tmp; b=b->next) {         //----------17.32%------------
    g_curr_pointer=b;
    test = distance(b,t);
   // _mm_prefetch(b->next,1);
    if (test < mindist) {
      mindist = test;
      min = b;
      } /* if */
