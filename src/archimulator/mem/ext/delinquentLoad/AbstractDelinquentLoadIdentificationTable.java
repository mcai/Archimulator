 ...
 ******************************************************************************/
package archimulator.mem.ext.delinquentLoad;
    ...
    public AbstractDelinquentLoadIdentificationTable(Thread thread) {
        ...
        thread.getBlockingEventDispatcher().addListener(CoherentCacheBeginCacheAccessEvent.class, new Action1<CoherentCacheBeginCacheAccessEvent>() {
            public void apply(CoherentCacheBeginCacheAccessEvent event) {
                if (!event.getCacheAccess().isHitInCache() && event.getAccess().getThread() == AbstractDelinquentLoadIdentificationTable.this.thread && event.getCache().isLastLevelCache() && event.getCacheAccess().getReference().getAccessType().isDownwardRead()) {
