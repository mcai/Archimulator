...
public class DynamicSpeculativePrecomputationCapability implements ProcessorCapability {
    ...
    public DynamicSpeculativePrecomputationCapability(Processor processor) {
        ...
        this.sliceCache = new EvictableCache<Boolean>(processor, "sliceCache", new CacheGeometry(SLICE_CACHE_CAPACITY, SLICE_CACHE_CAPACITY, 1), LRUPolicy.class, valueProviderFactory);
    ...
    private CacheMiss<Boolean> findInvalidLineAndNewMissInSliceCache(int address, CacheAccessType accessType, int set) {
        CacheReference reference = new CacheReference(null, null, address, this.sliceCache.getTag(address), accessType, set);

        for (int way = 0; way < this.sliceCache.getAssociativity(); way++) {
            CacheLine<Boolean> line = this.sliceCache.getLine(set, way);
            if (line.getState() == line.getInitialState()) {
                return new CacheMiss<Boolean>(this.sliceCache, reference, way);
            }
        }
    ...
    private class SliceInformationTable {
        ...
        private void storeSlice(Slice slice) {
            ...
            CacheMiss<Boolean> miss = findInvalidLineAndNewMissInSliceCache(slice.triggerPc, CacheAccessType.UNKNOWN, set);
            ...
            if (miss != null) {
                ...
                stateProvider.machInsts = machInsts;
        ...
        private void prepareRemoveSlice(Slice slice) {
            ...
            CacheLine<Boolean> lineFound = sliceCache.findLine(slice.triggerPc);
            if ((lineFound == null)) {
    ...
    private class PrecomputationThreadContext extends Context {
        ...
        protected StaticInstruction decode(int pc) {
            CacheLine<Boolean> lineFound = sliceCache.findLine(this.triggerPc);
            BooleanValueProvider stateProvider = lineFound != null ? (BooleanValueProvider) lineFound.getStateProvider() : null;

            if ((!(lineFound != null && stateProvider.machInsts != null))) {
                throw new IllegalArgumentException();
            }

            if (stateProvider.machInsts.containsKey(pc)) {
                return super.decode(stateProvider.machInsts.get(pc));
            } else {
