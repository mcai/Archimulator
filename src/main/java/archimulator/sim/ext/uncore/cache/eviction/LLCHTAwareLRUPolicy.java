...
public class LLCHTAwareLRUPolicy extends LRUPolicy<DirectoryControllerState> {
    ...
    public LLCHTAwareLRUPolicy(DirectoryController directoryController) {
        super(directoryController.getCache());

        this.llcHtRequestProfilingCapability = new HTLLCRequestProfilingCapability(directoryController);

        this.evictedMtLinesPerInterval = directoryController.getCache().getNumSets() * directoryController.getCache().getAssociativity() / 2;
    ...
    @Override
    public void handleInsertionOnMiss(CacheMiss<DirectoryControllerState> miss) {
