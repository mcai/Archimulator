...
public class HTLLCRequestProfilingHelper {
    ...
    public HTLLCRequestProfilingHelper(Simulation simulation) {
        this(simulation.getProcessor().getCacheHierarchy().getL2CacheController());
    }

    public HTLLCRequestProfilingHelper(final DirectoryController llc) {
        this.llc = llc;
        ...
        this.htLLCRequestVictimCache = new EvictableCache<HTLLCRequestVictimCacheLineState>(llc, llc.getName() + ".htLLCRequestVictimCache", llc.getCache().getGeometry(), CacheReplacementPolicyType.LRU, cacheLineStateProviderFactory);
        ...
        llc.getBlockingEventDispatcher().addListener(CoherentCacheServiceNonblockingRequestEvent.class, new Action1<CoherentCacheServiceNonblockingRequestEvent>() {
            public void apply(CoherentCacheServiceNonblockingRequestEvent event) {
                if (event.getCacheController().equals(HTLLCRequestProfilingHelper.this.llc)) {
        ...
        llc.getBlockingEventDispatcher().addListener(LastLevelCacheLineInsertEvent.class, new Action1<LastLevelCacheLineInsertEvent>() {
            ...
            public void apply(LastLevelCacheLineInsertEvent event) {
                if (event.getCacheController().equals(HTLLCRequestProfilingHelper.this.llc)) {
                    int set = event.getSet();
                    boolean requesterIsHT = BasicThread.isHelperThread(event.getAccess().getThread());
                    boolean lineFoundIsHT = HTLLCRequestProfilingHelper.this.llcLineHTRequestStates.get(set).get(event.getWay()).getThreadId() == BasicThread.getHelperThreadId();
        ...
        llc.getBlockingEventDispatcher().addListener(CoherentCacheLastPutSOrPutMAndDataFromOwnerEvent.class, new Action1<CoherentCacheLastPutSOrPutMAndDataFromOwnerEvent>() {
            ...
            public void apply(CoherentCacheLastPutSOrPutMAndDataFromOwnerEvent event) {
                if (event.getCacheController().equals(HTLLCRequestProfilingHelper.this.llc)) {
                    ...
                    if (lineFoundIsHT) {
                        int wayOfVictimCacheLine = findWayOfVictimCacheLineByHtRequestTag(event.getSet(), event.getTag());
        ...
        llc.getBlockingEventDispatcher().addListener(CoherentCacheNonblockingRequestHitToTransientTagEvent.class, new Action1<CoherentCacheNonblockingRequestHitToTransientTagEvent>() {
            ...
            public void apply(CoherentCacheNonblockingRequestHitToTransientTagEvent event) {
                if (event.getCacheController().equals(HTLLCRequestProfilingHelper.this.llc)) {
                    ...
                    int lineFoundThreadId = HTLLCRequestProfilingHelper.this.llcLineHTRequestStates.get(set).get(event.getWay()).inFlightThreadId;
...
//        this.setLRU(event.getSet(), vtLine.getWay());
    ...
    private void handleLineInsert3(LastLevelCacheLineInsertEvent event) {
        // case 3
        int wayOfVictimCacheLine = this.findWayOfVictimCacheLineByHtRequestTag(event.getSet(), event.getVictimTag());
...
//        }
        ...
        if (checkInvariantsEnabled) {
            ...
            for (int i = 0; i < this.llc.getCache().getAssociativity(); i++) {
                ...
                if (line.getState().isStable() && line.isValid() && this.llcLineHTRequestStates.get(set).get(i).getThreadId() == BasicThread.getHelperThreadId()) {
                    int wayOfVictimCacheLine = this.findWayOfVictimCacheLineByHtRequestTag(set, line.getTag());
    ...
    private void insertDataEntry(int set, int tag, int htRequestTag) {
        ...
        line.setTag(tag);
        htLLCRequestVictimCache.getReplacementPolicy().handleInsertionOnMiss(set, newMiss.getWay());
    }
...
//    }
    ...
    private int findWayOfVictimCacheLineByHtRequestTag(int set, int htRequestTag) {
        for (int way = 0; way < this.llc.getCache().getAssociativity(); way++) {
            CacheLine<HTLLCRequestVictimCacheLineState> line = this.htLLCRequestVictimCache.getLine(set, way);
            HTLLCRequestVictimCacheLineStateValueProvider stateProvider = (HTLLCRequestVictimCacheLineStateValueProvider) line.getStateProvider();
    ...
    private LRUPolicy<HTLLCRequestVictimCacheLineState> getLruPolicyForHtRequestVictimCache() {
        return (LRUPolicy<HTLLCRequestVictimCacheLineState>) this.htLLCRequestVictimCache.getReplacementPolicy();
    }
    ...
    public double getHtLLCRequestCoverage() {
        return (this.numMTLLCMisses + this.numUsefulHTLLCRequests) == 0 ? 0 : (double) this.numUsefulHTLLCRequests / (this.numMTLLCMisses + this.numUsefulHTLLCRequests);
    }

    public double getHtLLCRequestAccuracy() {
        return this.numTotalHTLLCRequests == 0 ? 0 : (double) this.numUsefulHTLLCRequests / this.numTotalHTLLCRequests;
    }
