...
/**
 * The event fired when the directory controller receives a "PutS (not last)" message.
 *
 * @author Min Cai
 */
public class PutSNotLastEvent extends DirectoryControllerEvent {
    ...
    /**
     * Create an event when the directory controller receives a "PutS (not last)" message.
     *
     ...
     */
    public PutSNotLastEvent(DirectoryController generator, CacheCoherenceFlow producerFlow, CacheController requester, int tag, MemoryHierarchyAccess access) {
    ...
    public String toString() {
        return String.format("[%d] %s: PutSNotLastEvent{id=%d, requester=%s, tag=0x%08x}", getBeginCycle(), getGenerator(), getId(), requester, getTag());
    }
}
