...
public class LastLevelCacheFindAndLockProcess extends FindAndLockProcess {
    public LastLevelCacheFindAndLockProcess(LastLevelCache cache, MemoryHierarchyAccess access, int tag, CacheAccessType cacheAccessType) {
        super(cache, access, tag, cacheAccessType);
    }

    @Override
    protected void evict(MemoryHierarchyAccess access, final Action onCompletedCallback) {
        getPendingActions().push(new MemWriteProcess(getCache(), access, cacheAccess).addOnCompletedCallback(new Action1<MemWriteProcess>() {
