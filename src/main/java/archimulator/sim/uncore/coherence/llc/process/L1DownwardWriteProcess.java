...
public class L1DownwardWriteProcess extends LastLevelCacheLockingProcess {
    private FirstLevelCache source;
    private DownwardWriteMessage message;

    public L1DownwardWriteProcess(LastLevelCache cache, final FirstLevelCache source, final DownwardWriteMessage message) {
        super(cache, message.getAccess(), message.getTag(), CacheAccessType.DOWNWARD_WRITE);
        ...
        this.getPendingActions().push(new ActionBasedPendingActionOwner() {
            @Override
            public boolean apply() {
                if (!findAndLockProcess.getCacheAccess().isHitInCache() && !getCache().isOwnedOrShared(tag)) {
                    getPendingActions().push(new MemReadProcess(getCache(), message.getAccess(), message.getTag()));
                }
        ...
        this.getPendingActions().push(new UpwardWriteProcess(getCache(), message.getAccess(), message.getTag(), source));
    }
