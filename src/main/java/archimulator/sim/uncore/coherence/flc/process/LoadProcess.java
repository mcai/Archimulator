...
public class LoadProcess extends CpuSideProcess {
    public LoadProcess(FirstLevelCache cache, final MemoryHierarchyAccess access) {
        super(cache, access, access.getPhysicalTag(), CacheAccessType.LOAD);
        ...
        this.getPendingActions().push(new ActionBasedPendingActionOwner() {
            ...
            public boolean apply() {
                if (!findAndLockProcess.getCacheAccess().isHitInCache()) {
                    getPendingActions().push(new DownwardReadProcess(getCache(), access, access.getPhysicalTag()).addOnCompletedCallback(new Action1<DownwardReadProcess>() {
