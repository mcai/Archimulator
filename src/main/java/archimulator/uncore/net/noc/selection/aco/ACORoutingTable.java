    ...
    public void update(int dest, Direction direction) {
        for(Pheromone pheromone : this.pheromones.get(dest).values()) {
            if(pheromone.getDirection() == direction) {
                pheromone.setValue(
                        pheromone.getValue()
                                + this.node.getNetwork().getSettings().getExperiment().getConfig().getReinforcementFactor() * (1 - pheromone.getValue())
                );
            } else {
                pheromone.setValue(
                        pheromone.getValue()
                                - this.node.getNetwork().getSettings().getExperiment().getConfig().getReinforcementFactor() * pheromone.getValue()
                );
            }
        }
    }
