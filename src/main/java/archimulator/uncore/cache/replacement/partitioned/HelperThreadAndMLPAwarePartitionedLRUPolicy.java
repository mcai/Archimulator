...
/**
 * Helper thread and MLP-aware partitioned least recently used (LRU) policy for the specified evictable cache.
 *
 * @param <StateT> the state type of the parent evictable cache
 * @author Min Cai
 */
public class HelperThreadAndMLPAwarePartitionedLRUPolicy<StateT extends Serializable> extends HelperThreadAwarePartitionedLRUPolicy<StateT> {
    private MLPAwareCachePartitioningHelper mlpAwareCachePartitioningHelper;

    /**
     * Create a helper thread and MLP-aware partitioned least recently used (LRU) policy for the specified evictable cache.
     *
     * @param cache the parent evictable cache
     */
    public HelperThreadAndMLPAwarePartitionedLRUPolicy(EvictableCache<StateT> cache) {
        super(cache);

        this.mlpAwareCachePartitioningHelper = new MLPAwareCachePartitioningHelper(cache);
        this.mlpAwareCachePartitioningHelper.setShouldIncludePredicate(set -> true);
    }
