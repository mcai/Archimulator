...
/**
 * The event fired when the directory controller receives a "PutM and data (from the owner)" message.
 *
 * @author Min Cai
 */
public class PutMAndDataFromOwnerEvent extends DirectoryControllerEvent {
    ...
    /**
     * Create an event when the directory controller receives a "PutM and data (from the owner)" message.
     *
    ...
    public String toString() {
        return String.format("[%d] %s: PutMAndDataFromOwnerEvent{id=%d, requester=%s, tag=0x%08x}", getBeginCycle(), getGenerator(), getId(), requester, getTag());
    }
}
